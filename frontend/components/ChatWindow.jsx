      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
        ...
        {messages.map((msg, index) => {
          ...
          if (isSystem) {
            return (
              <div key={index} className="flex justify-center">
                <div className={clsx(
                  "flex items-center gap-2 text-xs px-3 py-2 rounded-full border",
          ...
          return (
            <div
              key={index}
              className={clsx("flex gap-3 animate-fade-in", isSelf ? "flex-row-reverse" : "flex-row")}
            >
              {/* Avatar */}
              <div className="w-7 h-7 rounded-full bg-gradient-to-br from-brand-500 to-accent-purple flex items-center justify-center text-xs font-bold text-white flex-shrink-0">
                {(msg.sender_name || "U")[0].toUpperCase()}
              </div>
              <div className={clsx("flex flex-col gap-1 max-w-xs lg:max-w-md", isSelf ? "items-end" : "items-start")}>
                <div className="flex items-center gap-2 text-xs text-white/40">
                  {!isSelf && <span>{msg.sender_name || "Unknown"}</span>}
                  {msg.timestamp && <span>{format(new Date(msg.timestamp), "HH:mm")}</span>}
                </div>
                <div
                  className={clsx(
